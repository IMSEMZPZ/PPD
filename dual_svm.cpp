...
#pragma parallel for 
			...
			{
				int i = batch_idx * batch_size + idx;//获得shuffle_index中的全局索引
				...
				if (abs(pg) > tol)
				{
					...
					alpha[rand_id] += d;
					//w的更新
					//for (int k = train_data.index[rand_id]; k<train_data.index[rand_id + 1]; ++k) {//遍历一个sample i
					//	w[train_data.col[k]] += d*train_data.Y[rand_id] * train_data.X[k];
					//}
				}
